...
public:
		...
		: myData()
	{
		if (count != 0)
		{
			myData.myFirst = new Ty[count]();//宣告一個動態陣列大小是count 然後myfirst放到第一個
	...
	{
		int newsize = size() + 1, i, w = where - myData.myFirst;
		if (capacity() < newsize) {
			int newcap = capacity() * 1.5;
			if (newsize > newcap)
				newcap = newsize;

			pointer temp = new Ty[newcap]();
			int i = 0;
			for (;i < w;i++)
			...
			for (i++;i < newsize;i++)
				temp[i] = myData.myFirst[i - 1];

			delete[] myData.myFirst;
			myData.myFirst = temp;
			myData.myEnd = myData.myFirst + newcap;
	...
	vector& operator=(const vector& right)
	{
		...
		{
			int newsize = right.myData.myLast - right.myData.myFirst;
			if (newsize > capacity()) {
				int newCap = capacity() * 1.5;
				if (newCap < newsize)
					newCap = newsize;
				pointer buffer = new Ty[newCap]();
				for (int i = 0;i < newsize;i++) {
					buffer[i] = right.myData.myFirst[i];
				}

				delete[]myData.myFirst;
				myData.myFirst = buffer;
				myData.myLast = myData.myFirst + newsize;
				myData.myEnd = myData.myFirst + newCap;
			}
	...
	iterator erase(const_iterator where)
	{
		...
		{
				int newSize = size() - 1;
				int deleted = where - myData.myFirst;
				myData.myLast = myData.myFirst + newSize;
				for (int i = deleted; i < newSize; i++)
					myData.myFirst[i] = myData.myFirst[i + 1];
		}
		else
			return nullptr;
